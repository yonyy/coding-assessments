...
class TunnelRateLimiter(val maxRequests: Int, val windowSeconds: Long) {
     ...
     */
    fun recordRequest(tunnelId: String, timestampSeconds: Long): Boolean {
        val queue = acceptedRequests.getOrPut(tunnelId) { ArrayDeque() }
        while(queue.isNotEmpty() && queue.first() <= timestampSeconds - windowSeconds) {
            queue.removeFirst()
        }

        return if (queue.size < maxRequests) {
            queue.addLast(timestampSeconds)
            true
        } else {
            false
        }
     ...
     */
    fun remainingRequests(tunnelId: String, currentTime: Long): Int {
        val queue = acceptedRequests[tunnelId] ?: return maxRequests
        return maxOf(0, maxRequests - queue.count { it > currentTime - windowSeconds })
    }
}